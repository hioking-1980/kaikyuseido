// スプレッドシートのベースID（URLから抽出したキー）
const SPREADSHEET_KEY = '2PACX-1vTw7lXTJViUKW_BaGSR0Kmku33fn7zpnusbQhVKa4o6Hb2Ahk_l2StGfAiFS_TbKpUg9ftOXAminMSN'; 

// --- GID（シートID）を直接指定 ---
const RANKING_GID = '589878966'; // ユーザーランキングデータ
const INFO_GID = '1072760862'; // ホルダー数とフロア価格データ
...
// --- 初期化関数 (Tabletopの基本方式を使用) ---
function init() {
    
    // 1. 階級ランキングデータの取得
    Tabletop.init({
        key: SPREADSHEET_KEY,
        gid: RANKING_GID, // GIDを個別に指定
...
// --- データ処理関数 (Tabletopのデータ構造に対応) ---

function processRankingData(data) {
    // ... 以前と同じ processRankingData の中身を貼り付け ...
...
function updateTopInfo(data) {
    try {
        if (data.length > 0) {
            // Tabletopのデータ構造に合わせて、最初の2行からデータを取得
